...
if ($hassiteconfig) {
    ...
    if (!empty($cloudpoodllapiuser) && !empty($cloudpoodllapisecret)) {
        $tokeninfo = utils::fetch_token_for_display(
            $cloudpoodllapiuser,
            $cloudpoodllapisecret,
        );
        ...
        foreach ($cpcomponents as $cpcomponent) {
            ...
            if (!empty($cloudpoodllapiuser)) {
                $cloudpoodllapisecret = get_config(
                    $cpcomponent,
                    $apisecretsetting,
                );
        ...
        $showbelowapisecret = $OUTPUT->render_from_template(
            constants::M_COMPONENT . "/managecreds",
            $amddata,
        );
    }
    ...
    $nativelangdefault =
        $CFG->lang && array_key_exists($CFG->lang, $shortlangcodes)
            ? $shortlangcodes[$CFG->lang]
            : 0;
    $mainsettings->add(
        new admin_setting_configselect(
    ...
    // Custom ASR section heading.
    $mainsettings->add(
        new admin_setting_heading(
            constants::M_COMPONENT . "/customasrheading",
            get_string("customasrurl", constants::M_COMPONENT),
            "",
        ),
    );

    // Custom ASR endpoint URL.
    $mainsettings->add(
        new admin_setting_configtext(
            constants::M_COMPONENT . "/customasrurl",
            get_string("customasrurl", constants::M_COMPONENT),
            get_string("customasrurl_details", constants::M_COMPONENT),
            "",
            PARAM_URL,
        ),
    );
    ...
    $mainsettings->add(
        new admin_setting_configtext(
            constants::M_COMPONENT . "/customasrapikey",
            get_string("customasrapikey", constants::M_COMPONENT),
            get_string("customasrapikey_details", constants::M_COMPONENT),
            "",
            PARAM_TEXT,
        ),
    );
    ...
    $modalsettings = [
        0 => get_string("modaleditform_newpage", constants::M_COMPONENT),
    ...
    $layoutoptions = [
        constants::LAYOUT_AUTO => get_string(
            "layoutauto",
            constants::M_COMPONENT,
        ),
        constants::LAYOUT_HORIZONTAL => get_string(
            "layouthorizontal",
            constants::M_COMPONENT,
        ),
        constants::LAYOUT_VERTICAL => get_string(
            "layoutvertical",
            constants::M_COMPONENT,
        ),
        constants::LAYOUT_MAGAZINE => get_string(
            "layoutmagazine",
            constants::M_COMPONENT,
        ),
    ];
    ...
    $mainsettings->add(
        new admin_setting_heading(
            constants::M_COMPONENT . "/finishscreen",
...
{{/results}}
 ...
 <a class ="btn btn-secondary" href="{{{reattempturl}}}">{{#str}} tryagain, mod_minilesson {{/str}}</a> <br />';
    ...
    $otherapikeysettings = new admin_settingpage(
        "modsettingminilessonotherapikeys",
        $pagetitle,
        "moodle/site:config",
    );
    ...
    $audiochatsettings = new admin_settingpage(
        "modsettingminilessonaudiochat",
        $pagetitle,
        "moodle/site:config",
    );

    // Audio chat settings.
    $audiochatsettings->add(
        new admin_setting_heading(
            constants::M_COMPONENT . "/audiochat",
            get_string("audiochat", constants::M_COMPONENT),
            "",
        ),
    );
    ...
    for ($i = 0; $i < $maxprompts; $i++) {
        ...
        $label =
            get_string("instructionsprompt_header", constants::M_COMPONENT) .
            " " .
            ($i + 1);
        $details = "";
        $default =
            $i < $defaults
                ? get_string(
                    "audiochat:instructionsprompt" . ($i + 1),
                    constants::M_COMPONENT,
                )
                : "";
        $audiochatsettings->add(
        ...
        $label =
            get_string("instructionsprompt", constants::M_COMPONENT) .
            " " .
            ($i + 1);
        $default =
            $i < $defaults
                ? get_string(
                    "audiochat:instructionsprompt_dec" . ($i + 1),
                    constants::M_COMPONENT,
                )
                : "";
        $audiochatsettings->add(
    ...
    for ($i = 0; $i < $maxprompts; $i++) {
        ...
        $label =
            get_string("feedbackprompt_header", constants::M_COMPONENT) .
            " " .
            ($i + 1);
        $details = "";
        $default =
        ...
        $label =
            get_string("feedbackprompt", constants::M_COMPONENT) .
            " " .
            ($i + 1);
        $default =
            $i < $defaults
                ? get_string(
                    "audiochat:feedbackprompt_dec" . ($i + 1),
                    constants::M_COMPONENT,
                )
                : "";
        $audiochatsettings->add(
    ...
    $freespeakingsettings = new admin_settingpage(
        "modsettingminilessonfreespeaking",
        $pagetitle,
        "moodle/site:config",
    );

    $freespeakingsettings->add(
        new admin_setting_heading(
            constants::M_COMPONENT . "/freespeaking",
            get_string("freespeaking", constants::M_COMPONENT),
            "",
        ),
    );
    $maxprompts = constants::MAX_AI_PROMPTS;
    ...
    for ($i = 0; $i < $maxprompts; $i++) {
        ...
        $label =
            get_string("gradingprompt_header", constants::M_COMPONENT) .
            " " .
            ($i + 1);
        $details = "";
        $default =
            $i < $defaults
                ? get_string(
                    "freespeaking:gradingprompt" . ($i + 1),
                    constants::M_COMPONENT,
                )
                : "";
        $freespeakingsettings->add(
        ...
        $label =
            get_string("gradingprompt", constants::M_COMPONENT) .
            " " .
            ($i + 1);
        $default =
            $i < $defaults
                ? get_string(
                    "freespeaking:gradingprompt_dec" . ($i + 1),
                    constants::M_COMPONENT,
                )
                : "";
        $freespeakingsettings->add(
    ...
    for ($i = 0; $i < $maxprompts; $i++) {
        ...
        $default =
            $i < $defaults
                ? get_string(
                    "freespeaking:feedbackprompt" . ($i + 1),
                    constants::M_COMPONENT,
                )
                : "";
        $freespeakingsettings->add(
        ...
        $default =
            $i < $defaults
                ? get_string(
                    "freespeaking:feedbackprompt_dec" . ($i + 1),
                    constants::M_COMPONENT,
                )
                : "";
        $freespeakingsettings->add(
    ...
    $freewritingsettings = new admin_settingpage(
        "modsettingminilessonfreewriting",
        $pagetitle,
        "moodle/site:config",
    );
    $freewritingsettings->add(
        new admin_setting_heading(
            constants::M_COMPONENT . "/freewriting",
            get_string("freewriting", constants::M_COMPONENT),
            "",
        ),
    );
    $maxprompts = constants::MAX_AI_PROMPTS;
    ...
    for ($i = 0; $i < $maxprompts; $i++) {
        ...
        $default =
            $i < $defaults
                ? get_string(
                    "freewriting:gradingprompt_dec" . ($i + 1),
                    constants::M_COMPONENT,
                )
                : "";
        $freewritingsettings->add(
    ...
    for ($i = 0; $i < $maxprompts; $i++) {
        ...
        $default =
            $i < $defaults
                ? get_string(
                    "freewriting:feedbackprompt" . ($i + 1),
                    constants::M_COMPONENT,
                )
                : "";
        $freewritingsettings->add(
        ...
        $default =
            $i < $defaults
                ? get_string(
                    "freewriting:feedbackprompt_dec" . ($i + 1),
                    constants::M_COMPONENT,
                )
                : "";
        $freewritingsettings->add(
    ...
    // AI MANAGER.
    if (
        class_exists(aimanager::class) &&
        class_exists(\core_ai\manager::class)
    ) {
        $aimanagertitle = get_string("aimanager", constants::M_COMPONENT);
        $aimanagersettings = new admin_settingpage(
            "modsettingminilessonaimanager",
            $aimanagertitle,
            "moodle/site:config",
        );
        ...
        foreach (
            aimanager::get_action_options()
            as $actiontype => $actiondetails
        ) {
            $aimanagersettings->add(
                new admin_setting_configselect(
